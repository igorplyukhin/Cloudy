...
def append_to_upload_session(session_id, offset, data):
    ...
    return post_safely(url=f'{conf.DROPBOX_CONTENT_URL}/upload_session/append_v2', headers=headers, data = data)
...
def rm(cloud_path):
    ...
    headers['Content-Type'] = 'application/json'
    data = json.dumps({'path': cloud_path})
    return post_safely(url=f'{conf.DROPBOX_API_URL}/delete_v2', headers=headers, data=data)
